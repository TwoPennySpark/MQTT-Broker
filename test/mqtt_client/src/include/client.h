...
enum {CLEAN_SESSION_TRUE=1, CLEAN_SESSION_FALSE=0};

class MQTTClient: public tps::net::client_interface<mqtt_header>
{
public:
    ~MQTTClient() { if (is_connected()) {send_disconnect();} }
    ...
    void connect_server(const std::string& clientID, uint8_t cleanSession = CLEAN_SESSION_TRUE);
